...
for i, v in oAcc.FindAll([{Name:"minim", cs:false, mm:2}])
    result .= i ": " v.Dump() "`n"
MsgBox result
...
for i, v in oAcc.FindAll([{Role:9}],5)
    result .= i ": " v.Dump() "`n"
result .= "`nLast element matching Role `"9`":`n" 
result .= oAcc.FindFirst({Role:9, index:-1}).Dump()
result .= "`nSecond element matching Role `"9`":`n" 
result .= oAcc.FindFirst({Role:9, index:2}).Dump()
MsgBox result
...
MsgBox("Last element matching Name `"Save	Ctrl+S`" or Name `"Save As...	Ctrl+Shift+S`":`n" 
    . oAcc.FindFirst({or:[{Name:"Save	Ctrl+S"}, {Name:"Save As...	Ctrl+Shift+S"}], index:-1}).Dump())

; To find specific element object (for example to determine the path) use IsEqual
oEdit := oAcc[4,1,4]
MsgBox("oEdit element: " oEdit.Dump()
    . "`n`nFirst element matching oEdit: " oAcc.FindFirst({IsEqual:oEdit}).Dump()
    . "`n`nFound elements path: " oAcc.FindFirst({IsEqual:oEdit}).Path)
